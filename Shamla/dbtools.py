...
class MDBDataPageValidator:
    ...
    def handleMDBDataPage(self, buf):
        ...
        if self.dbversion == MDBDefinitionMarkers.VJET4:
            if MDBDataPageMarkers.ACCESS2KVERSIONSTRING in binascii.hexlify(
                    buf) or MDBDataPageMarkers.ACCESS97VERSIONSTRING in binascii.hexlify(buf):
                self.freespace = struct.unpack('<H', buf[
                                                     MDBDataPageMarkers.FREESPACE: MDBDataPageMarkers.FREESPACE + MDBDataPageMarkers.FREESPACELEN])
                tabledef = buf[MDBDataPageMarkers.TDEF: MDBDataPageMarkers.TDEF + MDBDataPageMarkers.TDEFLEN]
                self.numberOfRows = struct.unpack('<H', buf[
                                                        MDBDataPageMarkers.NOROWS2K: MDBDataPageMarkers.NOROWS2K + MDBDataPageMarkers.NOROWSLEN])[
                    0]
            ...
            if MDBDataPageMarkers.ACCESS97VERSIONSTRING in binascii.hexlify(buf):
                ...
                tabledef = buf[MDBDataPageMarkers.TDEF: MDBDataPageMarkers.TDEF + MDBDataPageMarkers.TDEFLEN]
                self.numberOfRows = struct.unpack('<H', buf[
                                                        MDBDataPageMarkers.NOROWS97: MDBDataPageMarkers.NOROWS97 + MDBDataPageMarkers.NOROWSLEN])[
                    0]
    ...
    def __getValue__(self, valuedata, index):
        ...
        partlen = struct.unpack('<H', valuedata[MDBDataPageMarkers.ZEROOFF: MDBDataPageMarkers.SHORTVAL])[
            0]  # length of block containing one value
