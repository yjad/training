...
def insert_row_list(conn, cursor, table_name, rec, ignoreUnique=False):

    question_marks = ','.join(list('?' * len(rec)))
    try:
        if ignoreUnique:
            cursor.execute('INSERT OR IGNORE INTO ' + table_name + ' VALUES (' + question_marks +')', tuple(rec))
        else:
            cursor.execute('INSERT INTO ' + table_name + ' VALUES (' + question_marks +')', tuple(rec))
        # conn.commit()
        return 0
    except sqlite3.Error as er:
        print_tk('SQLite error: %s' % (' '.join(er.args)))
        print_tk(f"Exception class is: {er.__class__}")
        print_tk('SQLite traceback: ')
        exc_type, exc_value, exc_tb = sys.exc_info()
        #print(traceback.format_exception(exc_type, exc_value, exc_tb))
        print_tk(f"{exc_type}, {exc_value}, {exc_tb}")
        exit(-1)
        return -1
...
# this version of exec_db_cmd for an open connection
def exec_db_cmd_conn(cmd, cursor): 
